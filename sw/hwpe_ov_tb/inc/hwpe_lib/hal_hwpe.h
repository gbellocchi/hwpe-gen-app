...
static inline void hwpe_addr_gen_a(
  ...
  unsigned int a_step)
{
  HWPE_WRITE(a_trans_size,    MAC_MDC_REG_A_TRANS_SIZE  );
  HWPE_WRITE(a_line_stride,   MAC_MDC_REG_A_LINE_STRIDE );
  HWPE_WRITE(a_line_length,   MAC_MDC_REG_A_LINE_LENGTH );
  HWPE_WRITE(a_feat_stride,   MAC_MDC_REG_A_FEAT_STRIDE );
  HWPE_WRITE(a_feat_length,   MAC_MDC_REG_A_FEAT_LENGTH );
  HWPE_WRITE(a_feat_roll,     MAC_MDC_REG_A_FEAT_ROLL   );
  HWPE_WRITE(a_loop_outer,    MAC_MDC_REG_A_LOOP_OUTER  );
  HWPE_WRITE(a_realign_type,  MAC_MDC_REG_A_REALIGN_TYPE);
  HWPE_WRITE(a_step,          MAC_MDC_REG_A_STEP        );
}
/* address generator hal - inputs */
static inline void hwpe_addr_gen_b(
  ...
  unsigned int b_step)
{
  HWPE_WRITE(b_trans_size,    MAC_MDC_REG_B_TRANS_SIZE  );
  HWPE_WRITE(b_line_stride,   MAC_MDC_REG_B_LINE_STRIDE );
  HWPE_WRITE(b_line_length,   MAC_MDC_REG_B_LINE_LENGTH );
  HWPE_WRITE(b_feat_stride,   MAC_MDC_REG_B_FEAT_STRIDE );
  HWPE_WRITE(b_feat_length,   MAC_MDC_REG_B_FEAT_LENGTH );
  HWPE_WRITE(b_feat_roll,     MAC_MDC_REG_B_FEAT_ROLL   );
  HWPE_WRITE(b_loop_outer,    MAC_MDC_REG_B_LOOP_OUTER  );
  HWPE_WRITE(b_realign_type,  MAC_MDC_REG_B_REALIGN_TYPE);
  HWPE_WRITE(b_step,          MAC_MDC_REG_B_STEP        );
}
/* address generator hal - inputs */
static inline void hwpe_addr_gen_c(
  ...
  unsigned int c_step)
{
  HWPE_WRITE(c_trans_size,    MAC_MDC_REG_C_TRANS_SIZE  );
  HWPE_WRITE(c_line_stride,   MAC_MDC_REG_C_LINE_STRIDE );
  HWPE_WRITE(c_line_length,   MAC_MDC_REG_C_LINE_LENGTH );
  HWPE_WRITE(c_feat_stride,   MAC_MDC_REG_C_FEAT_STRIDE );
  HWPE_WRITE(c_feat_length,   MAC_MDC_REG_C_FEAT_LENGTH );
  HWPE_WRITE(c_feat_roll,     MAC_MDC_REG_C_FEAT_ROLL   );
  HWPE_WRITE(c_loop_outer,    MAC_MDC_REG_C_LOOP_OUTER  );
  HWPE_WRITE(c_realign_type,  MAC_MDC_REG_C_REALIGN_TYPE);
  HWPE_WRITE(c_step,          MAC_MDC_REG_C_STEP        );
}
...
static inline void hwpe_addr_gen_d(
  ...
  unsigned int  d_step)
{
  HWPE_WRITE(d_trans_size,    MAC_MDC_REG_D_TRANS_SIZE  );
  HWPE_WRITE(d_line_stride,   MAC_MDC_REG_D_LINE_STRIDE );
  HWPE_WRITE(d_line_length,   MAC_MDC_REG_D_LINE_LENGTH );
  HWPE_WRITE(d_feat_stride,   MAC_MDC_REG_D_FEAT_STRIDE );
  HWPE_WRITE(d_feat_length,   MAC_MDC_REG_D_FEAT_LENGTH );
  HWPE_WRITE(d_feat_roll,     MAC_MDC_REG_D_FEAT_ROLL   );
  HWPE_WRITE(d_loop_outer,    MAC_MDC_REG_D_LOOP_OUTER  );
  HWPE_WRITE(d_realign_type,  MAC_MDC_REG_D_REALIGN_TYPE);
  HWPE_WRITE(d_step,          MAC_MDC_REG_D_STEP        );
}

/* basic hal */

static inline void hwpe_trigger_job() {
  HWPE_WRITE(0, MAC_MDC_REG_TRIGGER);
}

static inline int hwpe_acquire_job() {
  return HWPE_READ(MAC_MDC_REG_ACQUIRE);
}

static inline unsigned int hwpe_get_status() {
  return HWPE_READ(MAC_MDC_REG_STATUS);
}

static inline void hwpe_soft_clear() {
  volatile int i;
  HWPE_WRITE(0, MAC_MDC_REG_SOFT_CLEAR);
}
...
static inline void hwpe_bytecode_set(unsigned int offs, unsigned int value) {
  HWPE_WRITE(value, MAC_MDC_REG_BYTECODE+offs);
}
